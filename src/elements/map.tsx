...
function IsiMap(props: {result: IResultManuscriptList}) {
    ...
    return (
      <div className="resultMap">
                      ...
                      {props.result.manuscripts.map((item: IResultManuscript) => {
                          if  (item.library[0].latitude !== 0 && !isNaN(item.library[0].latitude)) {
                              ...
                              return  (<Marker position={position}>
                                  <Popup><span>{item.library[0].place_name}<br/></span><span className="hcClickable" onClick={() => {
          ...
          <div className="mapSwitch"><div className={org} onClick={() => {setModernPlace(false)}}>Origin</div><div className={lib} onClick={() => {setModernPlace(true)}}>Library</div></div>
      </div>
    );
}
